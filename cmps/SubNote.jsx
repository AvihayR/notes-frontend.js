...
export default function SubNote({ note, i, onDeleteNote }) {
    ...
    return (
        <div className="group sub-note-container relative flex justify-center items-center h-19 text-2xl font-light pl-18 text-blue-900 border-b-blue-200 border-b-1">
            <input type="checkbox" title={noteObj.completed ? 'Mark not completed' : 'Mark completed'} name={i} onChange={handleCheck} checked={noteObj.completed} className=" absolute cursor-pointer left-5 z-20 w-5 h-5 rounded-2xl mb-2.5 accent-amber-400" />
            <textarea placeholder="Your note here.." className={`w-full h-2/3 resize-none focus:outline-0 border-0 decoration-amber-400 ${noteObj.completed && 'line-through'}`} value={noteObj.desc} onChange={handleChange} onBlur={handleBlur} />
            <button title="Delete note" onClick={() => {
                setNoteObj({ desc: '', note_id: null, completed: false })
                onDeleteNote(noteObj.note_id)
            }
            } className={`p-1.5 cursor-pointer opacity-0 transition duration-300 ease-in-out group-hover:${noteObj.note_id && 'opacity-100'}`}>
