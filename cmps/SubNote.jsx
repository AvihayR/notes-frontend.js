export default function SubNote({ note, i, onDeleteNote, handleChange, handleBlur, handleCheck }) {
    ...
    return (
        <div className="group sub-note-container relative flex justify-center items-center h-19 text-2xl font-light pl-18 text-blue-900 border-b-blue-200 border-b-1">
            <input type="checkbox" title={note.completed ? 'Mark not completed' : 'Mark completed'} name={i} onChange={() => handleCheck(note)} checked={note.completed} className=" absolute cursor-pointer left-5 z-20 w-5 h-5 rounded-2xl mb-2.5 accent-amber-400" />
            ...
            <button title="Delete note" onClick={() => { onDeleteNote(note.note_id) }} className={`p-1.5 cursor-pointer opacity-0 transition duration-300 ease-in-out group-hover:${note.note_id && 'opacity-100'}`}>
